...
<section id="content">
    <section class="hbox stretch">
        <section class="vbox">
            <section class="scrollable wrapper" id="bjax-target">   
		...
		<{section name=times loop=$search_bug}>
			...
			<span class="label <{if $commited_bug[times].status eq "0"}>label-info">新发布</span><{/if}>
					 <{if $commited_bug[times].status eq "1"}>label-danger">修复中</span><{/if}>
					 <{if $commited_bug[times].status eq "2"}>label-success">已修复</span><{/if}>
					 <{if $commited_bug[times].status eq "3"}>label-default">已关闭</span><{/if}>

                      <a href="<{$SITE_DOMAIN}>/app/feedback/display_bug.php?bug_name=<{$search_bug[times].bugid}>" class="h5"><{$search_bug[times].name}></a>
                    </div>
                  </article>
		<{/section}>
		...
		<{section name=times loop=$waited_bug}>
                  <article class="media">
                    <div class="media-body">                        
                      <a href="<{$SITE_DOMAIN}>/app/feedback/display_bug.php?bug_name=<{$waited_bug[times].bugid}>" class="h5"><{$waited_bug[times].name}></a>
                    </div>
                  </article>
                  <div class="line pull-in"></div>
		<{/section}>
		...
		<{section name=times loop=$fixed_bug}>
                  <article class="media">
                    <div class="media-body">                        
                      <a href="<{$SITE_DOMAIN}>/app/feedback/display_bug.php?bug_name=<{$fixed_bug[times].bugid}>" class="h5"><{$fixed_bug[times].name}></a>
                    </div>
                  </article>
                  <div class="line pull-in"></div>
		<{/section}>
		...
		<{section name=times loop=$commited_bug}>
		      ...
		      <span class="label <{if $commited_bug[times].status eq "0"}>label-info">新发布</span><{/if}>
					 ...
					 <{if $commited_bug[times].status eq "2"}>label-success">已修复</span><{/if}>
					 <{if $commited_bug[times].status eq "3"}>label-default">已关闭</span><{/if}>
                      <a href="<{$SITE_DOMAIN}>/app/feedback/display_bug.php?bug_name=<{$commited_bug[times].bugid}>" class="h5"><{$commited_bug[times].name}></a>
                    </div>
                  </article>
                  <div class="line pull-in"></div>
		<{/section}>
...
<script src="<{$SITE_DOMAIN}>/js/jquery.min.js"></script>
			...
			<h5 class="modal-title text-center" id="tips-modal-title"></h5>
		    </div>
	      ...
	      <script type="text/javascript">
                 $(document).ready(function() {
                     $('#btn-send-bug').click(function () {
                        ...
                        {
                                $('#tips-modal-title').html("标题、bug描述或bug复现方法不能为空!");
