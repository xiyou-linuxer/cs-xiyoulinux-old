...
!function($) {
    ...
    var validateField = function(field, valid) { // 验证字段
        ...
        if(crule){
		//console.log(el.val());
            if( ! eval(crule[0]).test(el.val()) && el.val().length!=0) {
                error = true;
                errorMsg =msg;
            }
        ...
        if (error) {
             ...
             if ( len<=0) {                
                el.after('  <a  style="visibility:hidden;" data-placement="bottom" data-content="'+errorMsg+ '"  data-toggle="popover" data-html="true" href="#" >aa</a> ');
             };
             el.next().popover("show"); 
             var pop=el.parent().find(".popover"),pos=pop.offset();
             pos.top=pos.top-el.next().height();
	     pos.left =  pos.left + pop.width()/2;
             pop.offset(pos);             
             //pop.popover({trigger:'focus'}); 
             if ( cls  ) pop.addClass("btvdclass");
             $("#submit-info").attr("disabled","true");
             $("#person-info input").not(el).attr("disabled", "true");
          
        } else {
            controls.popover("hide");
            $("a[style='visibility:hidden;']").remove();
            $("#submit-info").removeAttr("disabled");
            $("#person-info input").not(this).removeAttr("disabled");
        }
        return !error;
    };
